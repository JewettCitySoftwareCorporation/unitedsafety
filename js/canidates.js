...
//     💫 ⭐️ 🌟 ✨ ⚡️ ☄️ 💥 🔥 🌪
     ...
     var firebaseConfig = {
        apiKey: "AIzaSyD5L1nBehTdbUAXzssuDpK2gR3huDiwSZ0",
        authDomain: "bice-form.firebaseapp.com",
        databaseURL: "https://bice-form.firebaseio.com",
        projectId: "bice-form",
        storageBucket: "bice-form.appspot.com",
        messagingSenderId: "536358896289",
        appId: "1:536358896289:web:dcd42dfac47eb789f666e0",
        measurementId: "G-R6D8H1GY9G"
      };
      // Initialize Firebase   https://bice-form.firebaseio.com/record
      firebase.initializeApp(firebaseConfig);
      //firebase.analytics();
// Get a reference to the database service
var dbRef = firebase.database();
var recordsRef = dbRef.ref('record');
var recentPostsRef = firebase.database().ref('record').limitToLast(100);
var this_record;
var tablz =document.querySelector('#tble');
//var row = tablz.insertRow(0);

firebase.database().ref('record').once('value', function(snapshot) {

    if(snapshot.exists()){
        var content = '';
        this_record=snapshot.val();
        
const editor = new JSONEditor(container, options, this_record)
        ...
        snapshot.forEach(function(data){
            ...
            console.log("IN SNAP:"+data.val());
            
            content += '<tr>';
            content += '<td>' + data.val() + '</td>'; //column1
            content += '<td>' + data.val() + '</td>';//column2
            content += '</tr>';
        });
       // tablz..append(content);
    }
